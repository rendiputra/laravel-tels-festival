...
// ADMIN CONTROLLERS
    ...
    public function user_add()
    {
        $lomba = DB::table('bidang_lomba')
            ->get();
        return view('admin.user_add',['lomba'=>$lomba]);
    }
    public function user_add_act(Request $req)
    {  
        $new = new User;
        $new->name = $req->name;
        $new->nama_sekolah = $req->nama_sekolah;
        $new->alamat_sekolah = $req->alamat;
        $new->contact_person = $req->no_telfon;
        $new->email = $req->email;
        $new->email_verified_at = date('Y-m-d');
        $new->password =   Hash::make($req->password);

        if($new->save()){
            if(count($req->lomba) > 0){

                $z = DB::table('users')->where('email',$req->email)->first();
                
                for ($i=0; $i < count($req->lomba) ; $i++) { 
                    $l = new pendaftaran;
                    $l->id_user = $z->id;
                    $l->id_bidang_lomba = $req->lomba[$i];
                    $l->tgl_kadaluarsa = date("Y-m-d", strtotime("+ 3 day"));
                    $l->kode_unik = rand(1,999);
                    $l->kode_daftarulang = $this->random_string(10);
                    $l->pembayaran = 1;
                    $l->save();
                }
            }

            return back()->with('sukses','Berhasil membuat user!');
        }
        return back();
    ...
    public function user_manage($id_user)
    {
        $data = DB::table('users')
                    ->where('id',$id_user)
                    ->first();
        ...
        $data_peserta = DB::table('data_pendaftar')
                ->get();
    ...
    public function user_delete($id_user)
    {
        ...
        $pendaftaran = DB::table('pendaftaran')->where('id_user',$id_user)->get();
        foreach($pendaftaran as $d){
            DB::table('data_pendaftar')->where('id_daftar',$d->id_daftar)->delete();
        }
        $pendaftaran = DB::table('pendaftaran')->where('id_user',$id_user)->delete();
        $user->delete();

        return redirect('/cup/admin/user');
